...
class HrZoneDial extends Ui.Drawable {
    ...
    function setHrZones(hrZones) {
        ...
        self.hrZoneAmount = (360 / (hrZones.count() - 1)) - HrZoneSeparation;
        calculateHrZoneBounds();
    }

    function calculateHrZoneBounds() {
        for (var zone = 1; zone < self.hrZones.count(); zone++) {
            self.hrZones.Properties[zone][:arcStart] = HrZoneStart - ((zone-1) * (self.hrZoneAmount + HrZoneSeparation));
    ...
    function draw(dc) {
        ...
        for (var zone = 1; zone <= self.hrZoneActive; zone++) {
